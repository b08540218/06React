...
1.props라는 대표 매개변수를 사용한다. 이때는 2개 이상의 인수를 객체형태로
  받을 수 있으므로 'propls.속성명'과 같이 사용한다.
2.인수를 개별 변수로 전달받는다.
  {매개변수1, 매개변수2...N}
 */
...
// 리듀서 함수 선언
const rendcer = (state, action) =>{
  switch (action.type) {
    case 'add':
      /* 학생 이름을 액션객체를 통해 읽어온다. (dispatch함수에서 전달하는
      액션객체를 참조) */
      ...
      case 'mark':
        return{
          // 학생수는 변함 없음
          count: state.count,
          /** 출력처리는 학생수에는 변함이 없으므로 map을 사용한다.
...
function App() {
  ...
  데이터 객체로 정의*/
  const [studentInfo, dispatch] = useReducer(rendcer, initialState);
  return (<>
<div className="App">
  ...
  <button onClick={()=>{
    if (name.trim()) {
      dispatch({type:'add', param:{name : name.trim() }});

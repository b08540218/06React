...
function Top(props) {
  return(
    <h2><a href='/' onClick={(event)=>{
      ...
      /* 프롭스로 전다로딘 함수를 호출한다. 이때 인수로 both를 전달하여
...
function App() {
  ...
  let contents = '';
  if (mode==='front') {
    // mode 값이 front면 MyCont1 컴포넌트만 변수에 저장후 렌더링 한다.
    contents = <>
    {/* 각 컴포넌트에서는 myModeChange라는 프롭스를 전달하게 되는데,
    스테이트로 정의한 mode의 값을 매개변수를 통해 변경하는 가능을
    가지고 있다. */}

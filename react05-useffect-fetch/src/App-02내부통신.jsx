...
const GlobalTop = (props) =>{
  ...
  }, []); // , [] <=의존성 배열 삭제시 렌더링이 멈추지 않는다.
  /** useEffect의 두번째 인자인 의존성 배열에 빈 배열을 추가한다.
  ...
  for (let i = 0; i < myList.length; i++) {
    ...
    listTag.push(
      <li key={data.id}>
        ...
        dataset.id를 통해 읽어올 수 있다. 이 속성에 게시물의 일련변호인
        num을 설정하고 있다. */}
...
function App() {   
  ...
  return (<>
    ...
    <div className="App">
      <h2>React - 내부서버통신</h2>
      {/* 클릭시 내부에 저장된 dto.json파일의 get방식으로 요청한 후
      콜백데이터를 받아오는 기능의 함수를 프롭스로 전달한다.
      자식 컴포넌트는 이 함수를 호출할때 게시물의 일련벼호를 인수로
      전달한다. */}
